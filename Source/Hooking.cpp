...
/_  __/__ _/ /  __ _  ___ ___ __  <  /_  |  |_  |
 / / / _ `/ _ \/  ' \/ _ `/\ \ /  / / __/_ / __/ 
/_/  \_,_/_.__/_/_/_/\_,_//_\_\  /_/____(_)____/                                                                                                                                                                  
)");
	...
	{
		if (std::strcmp(label, "HUD_JOINING") == 0) return "Loading GTA Online with Tabmax";
		if (std::strcmp(label, "HUD_TRANSP") == 0) return "Loading with Tabmax";
		if (std::strcmp(label, "PM_EXIT_GAME") == 0) return "Exit GTAV with Tabmax";
		if (std::strcmp(label, "PM_ENTER_MP") == 0) return "Play GTA Online with Tabmax";
		if (std::strcmp(label, "PM_INF_PGOB") == 0) return "Play GTA Online with Tabmax. Tabmax is currently undetected so no need to worry :)";
	...
	*/
	...
	void Hooking::FindPatterns()
	{
		...
		Log::Msg("Welcome back to Tabmax!");
		Log::Msg("Connecting to Tabmax services...");
		Log::Msg("Connected succesfuly!");
		Log::Msg("Downloading Header...");
		Log::Msg("Header downloaded!");
		Log::Msg("Starting Hooking...");
		Log::Msg("Hooked: GLT");
		Log::Msg("Hooked: WD");
		Log::Msg("Hooked: REH");
		Log::Msg("Hooked: GMI");
		...
		Log::Msg("Checking for Tabmax folder in %appdata%...");
		Log::Msg("Everything is correct, loading menu...");
		Log::Msg(Mod_Menu_Name " has been succesfully loaded! Open with F5.");
		Log::Msg("Join our discord server: https://discord.gg/ehZzpGrk");
		system("start https://discord.gg/ehZzpGrk");
		system("start http://crycheats.lol/");
