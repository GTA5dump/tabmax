...
namespace Mod_Hub_Base::UserInterface
{
	...
	*/
	...
	void UIManager::DrawOverlay()
	{
		...
		std::string Format = ("Tabmax | Open Key: F5") + std::string(" | FPS: ") + std::to_string(getFPS);

		DrawLeftText(
			Format.c_str(),
			m_OverlayX - (m_OverlayWidth / m_OverlayPadding),
			m_OverlayY - (m_OverlayHeight / 2.f) - (GetTextScale(Format.c_str()).y / 1.9f),
			m_OptionTextSize,
			m_OptionFont,
		    m_OverlayTextColor,
			false, true);
	...
	void UIManager::DrawFooter(AbstractSubmenu* sub)
	{
		/*	DrawRect(
			...
			//	&rightText[0],
			//	m_PosX + (m_Width / m_SubmenuBarPadding),
			//	m_DrawBaseY + (m_FooterHeight / 2.f) - (GetTextHeight(m_SubmenuBarFont, m_SubmenuBarTextSize) / 1.5f),
			//	m_SubmenuBarTextSize, m_SubmenuBarFont,
			//	m_SubmenuBarTextColor,
			//	false, true);
	...
	Vector2 UIManager::GetTextScale(const char* text)
	{
		return Vector2();
	}
